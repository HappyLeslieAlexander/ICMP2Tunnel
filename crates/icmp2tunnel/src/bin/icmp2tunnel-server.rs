...
fn validate_config(cfg: &ServerConfig) -> Result<(), String> {
    ...
    if cfg.max_sessions == 0 || cfg.max_streams_per_session == 0 || cfg.max_sessions_per_peer == 0 {
        return Err("session limits must be greater than 0".to_string());
    }
...
fn audit(record: AuditLogRecord) {
    info!(
        peer = %record.peer,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(bind_addr = %cfg.bind_addr, max_sessions = cfg.max_sessions, max_streams_per_session = cfg.max_streams_per_session, max_sessions_per_peer = cfg.max_sessions_per_peer, "server starting");

    let shutdown = Arc::new(AtomicBool::new(false));
    let signal = Arc::clone(&shutdown);
    ctrlc::set_handler(move || signal.store(true, Ordering::SeqCst))?;

    let mut limiter = RateLimiter::new(cfg.rate_limits.clone(), Instant::now());
    let started = Instant::now();
    while !shutdown.load(Ordering::SeqCst) {
        let peer = cfg.bind_addr;
        if !cfg.peer_acl.allows(peer) {
            warn!(peer = %peer, "peer rejected by ACL");
            thread::sleep(Duration::from_millis(100));
            continue;
        }

        let target = SocketAddr::from(([127, 0, 0, 1], 8080));
        if !cfg.target_acl.allows_socket(target) {
            warn!(target = %target, "target rejected by ACL");
            thread::sleep(Duration::from_millis(100));
            continue;
        }

        if !limiter.allow(Instant::now(), 64) {
