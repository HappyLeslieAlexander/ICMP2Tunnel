...
use icmp2tunnel::icmp::{
    build_echo_request_for, parse_echo_packet_for, IcmpAddr, IcmpProtocol, IcmpSocket,
};
...
fn load_config(cli: &Cli) -> Result<ClientConfig, Box<dyn std::error::Error>> {
    ...
    Ok(ClientConfig {
        ...
        server_addr: IcmpAddr::parse(&file.server_addr)?,
...
impl TunnelClient {
    fn new(cfg: &ClientConfig) -> io::Result<Self> {
        ...
        let icmp_protocol = cfg.server_addr.protocol();
        let socket = IcmpSocket::raw_for(icmp_protocol)?;
    ...
    fn exchange(&self, frame: Frame) -> io::Result<Frame> {
        ...
        let request = build_echo_request_for(self.icmp_protocol, self.icmp_identifier, icmp_seq, &sealed);
            ...
            loop {
                match socket.recv_from(&mut buf) {
                    Ok((n, src)) => {
                        ...
                        let echo = match parse_echo_packet_for(self.icmp_protocol, &buf[..n]) {
                            Ok(echo) => echo,
                            Err(_) => continue,
                        };
                        if echo.kind != self.icmp_protocol.reply_type()
